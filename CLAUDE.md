...
**Four such harnesses exist, and they are not all the same shape.** Three drive `dist/index.js` over real stdio the way an MCP client does, so **`deno task build` first** or you are testing the previous build, and each of those three forces `MCP_COMFYUI_AUTO_LAUNCH=0`, so none of them can start a GPU process while you are aiming at another machine.
...
**The fourth, `scripts/smoke-autolaunch.mjs`, is deliberately built the opposite way and is not in the table above.** It drives the **compiled binary**, `dist/mcp-comfyui` — so `deno task build && deno task compile` first, not just `build` — and it deliberately **leaves `MCP_COMFYUI_AUTO_LAUNCH` at its default (on)**, because auto-launch with nothing configured is the one thing it exists to prove. **It WILL start a GPU process, on this machine, and it does not stop it when it finishes** — read that as a safety note before running it against a box you don't mean to wake up, not as a gap to file. The caller stops ComfyUI, before and after, with `comfy --skip-prompt --json stop`.
...
**The live half of that audit, closed 2026-08-30** against the Windows RTX 4070 (12 GiB) once Tailscale was up — ComfyUI **0.33.3**, still at `198.51.100.10:8188`, through `dist/index.js` under `node` over real stdio. Both live harnesses passed end to end, and **every claim they exercise reproduced**:

- **`scripts/smoke-remote-artifacts.mjs`: all checks passed in 103 s.** `run_workflow {wait: true}` with **no `fetch_outputs`** returned `outputs.fetched` naming `~/.cache/mcp-comfyui/fetched/<prompt_id>/Chroma-Radiance_00013_.png`, which really existed at **1,559,836 bytes** — a genuine 1024×1024 8-bit RGB PNG, and the same byte count as the 2026-08-22 run, since it is the same graph at the same stored seed. `local_paths` stayed `{}`, correctly. A second `get_job` left the mtime unchanged, so the copy was reused.
- **`scripts/smoke-sweep.mjs`: all checks passed.** Three variants, three distinct `prompt_id`s, `failed: []`, zipped not crossed. ComfyUI's own `/history` for variant 0 carries `"noise_seed": 18446744073709551615` and the rounded `…552000` appears nowhere — read from the **raw response text**, never `JSON.parse`. `get_job` answered for all three with **no `host`**, each `host_source: "ledger"`. All three then completed and produced three **different** images — `Chroma-Radiance_00014_.png`, `_00015_`, `_00016_` — from seeds `18446744073709551615`, `12345` and `67890`, each host-side history carrying exactly the seed asked for. One sweep, three renders, three exact seeds.
- **Ground truth #41 re-measured directly.** `curl -I` on the artifact's `/view` URL returned `Content-Length: 1559836` with no `Transfer-Encoding: chunked`, so `fetchOutputs.ts`'s pre-check really does decline an oversized artifact without moving it.
- **Ground truth #24 reproduced verbatim** — `workflow slots` against the remote still fails `cql_no_graph`, *"Refusing to fetch object_info from non-loopback host … in local mode (potential SSRF)"*. `--input` remains the only schema source that works off-box.
- **Ground truth #43–#46 and #51 reproduced exactly, byte counts included.** `vary --out-dir --input` on `video_wan2_2_14B_i2v` wrote 84,918 / 84,899 / 84,899 bytes with the exact 2^64−1 digits in variant 0 and the rounded value nowhere; `variants` was present and **null**; **`stale` was absent and `warnings` empty**, which is #51, the defect a live run found and no fixture could; and a comma list was refused with `workflow_slot_invalid`.
- **Ground truth #32–#33 reproduced exactly**, with nothing listening: `workflow notes` returned `count: 2` in **0.35 s**, the payload keys were `{workflow, count, notes}` with **no `id`**, each note carried all seven keys with `subgraph: null`, and `comfy workflow notes --json` still fails **exit 2** for putting a root flag after the subcommand.
- **Per-host node definitions are doing the work they exist for.** Three caches sit on disk, one per `host:port`, and the two hosts disagree exactly as designed: `CheckpointLoaderSimple.ckpt_name` offers `ltx-2.3-22b-dev-fp8.safetensors` on the remote and `stable_audio_3_medium.safetensors` on this Mac; the remote lists 6 VAEs and 11 UNETs against this machine's 3 and 4.

**Two things moved on the remote, recorded as a new line rather than an edit.** Its `output_directory` is now a different Windows drive path (shown here as `F:\ComfyUI\output`; it was `D:\ComfyUI\output` in the 2026-08-22 entry), which changes nothing — `resolveArtifactPath` refuses a non-local instance before it ever looks at the path. And its `workflows` userdata directory now answers **200 with `[]`** where it once 404'd, because the directory has since been created and left empty. Both are "this host has no saved workflows", `listRemoteWorkflows` already handles each arm, and ground truth #22 now records both shapes instead of only the 404.

**The audit found a real tailnet address in a tracked test file, which is the leak class this project already wrote a rule about.** `tests/target.test.ts` carried one in its injected interface table, in a test named *"this machine's own Tailscale address"* — and it sat in the **same /24 as the live remote**, which is what identified it as real rather than invented. The old value is deliberately not quoted here or in the CHANGELOG: naming it would republish the thing being removed, and the shape is the whole lesson. It is now `100.64.0.1`, the first address of RFC 6598's shared address space: the same 100.64.0.0/10 range Tailscale assigns from, so the case still tests what it was written to test, but belonging to nobody. **The 2026-08-22 PII sweep missed it because it was a `.ts` file**, which is the exact mistake that sweep's own write-up warns about — sweep `git ls-files`, never just `*.md`. RFC 1918 addresses elsewhere in the suite (`192.168.1.50`, `10.0.0.x`) were left: that space is non-unique by design and names no machine, where a tailnet address is unique per node and does.

**A cold render is three times the warm figure, and neither number is wrong.** The single run took **101.6 s** against the 31.3 s recorded on 2026-08-22 for the same workflow on the same box. The difference is model load, not regression — worth knowing because the 2026-08-22 entry's warning still holds and bites harder: a real render outlasts an MCP client's 60 s default request timeout, and the first render after a cold start outlasts it by a lot.

**Self-sufficient CLI resolution, verified 2026-09-19** with `comfy` stopped and confirmed down (`comfy --skip-prompt --json stop` answering `no_background_server`), through the freshly rebuilt `dist/mcp-comfyui` over real stdio (`scripts/smoke-autolaunch.mjs`), spawned with **nothing configured**: no `COMFY_BIN`, and a `PATH` of exactly `/usr/bin:/bin:/usr/sbin:/sbin` — a GUI client's bare launchd `PATH`, the one environment this whole feature exists for. comfy-cli lives at `~/.local/bin/comfy`, which is nowhere on that `PATH`.

- **The first run failed, and found a real defect the whole test suite could not.** `comfy_status` reported `cli.source: "not_found"` with `searched` naming `~/.local/bin/comfy` and the other three candidates — the exact file that exists and is executable. Cause: `binary.ts`'s `isExecutable` calls `accessSync(path, X_OK)`, and under `deno compile`'s sandbox that call needs `--allow-sys=uid` (and, for the general case, `gid`) to read the calling process's own identity before it can compare it against the file's owner bits. The `compile` task granted only `--allow-sys=networkInterfaces,homedir`, so every real candidate threw `NotCapable`, `isExecutable`'s catch-all turned that into `false`, and discovery always fell through to `not_found` — **in the one build (`dist/mcp-comfyui`) that ships to users with no Node installed.** The plain-Node build (`dist/index.js`, no Deno sandbox to satisfy) resolved the identical path correctly on the first try, which is what isolated the cause to the compiled binary's permission set rather than to `binary.ts`'s logic. No test here could have caught this: every test that reaches `resolveComfyBinary` injects a fake `isExecutable`, and none calls `defaultBinaryDeps()` against a real file. Fixed by adding `uid,gid` to the `compile` task's `--allow-sys` list in `deno.json`; confirmed with a standalone compiled Deno script reproducing the exact `NotCapable` before the fix and a clean pass after.
- **With the fix applied and the binary recompiled, the full harness passed — criteria 1, 3 and 4 together.** `comfy_status` reported `cli: {"path": "~/.local/bin/comfy", "source": "discovered", "path_repaired": true}` and `running: false` before the run. `run_workflow {workflow: "image_z_image_turbo", wait: false}` was accepted and returned a `prompt_id`; `get_job` polling reported `status: "completed"` after **78.9 s** — a cold ComfyUI launch plus the render, from a standing start. `outputs.local_paths` was populated: `{"http://127.0.0.1:8188/view?filename=z-image_00003_.png&subfolder=&type=output": "~/ComfyUI-Installs/ComfyUI/ComfyUI/output/z-image_00003_.png"}`, and that file really exists — **1,223,071 bytes, a genuine 1024×1024 8-bit RGB PNG** (`file`/`sips` both agree), at a path this server derived from the instance it had itself just launched.
- **Criterion 2, checked separately:** with `COMFY_BIN=/nope/comfy` set explicitly, `comfy_status`'s `cli` block reported `{"path": "/nope/comfy", "source": "COMFY_BIN", "path_repaired": true}` — discovery is not consulted once an override is named — and `run_workflow` failed with exactly
  ```json
  {"error": {"kind": "comfy_unavailable", "message": "could not start the comfy CLI at `/nope/comfy`: spawn /nope/comfy ENOENT\nInstall comfy-cli and put it on PATH, or set COMFY_BIN to the binary's full path.", "binary": "/nope/comfy"}}
  ```
  with **no `searched` key** — an explicit bad binary is an error to report, never an invitation for discovery to rescue it.
- ComfyUI was stopped again afterward (`comfy --skip-prompt --json stop` reporting `stopped: true`), leaving the machine as found.

**Follow-up, same day: the defect above was narrower than the bug, and review of this entry caught it.** Reproduced directly with `deno run --allow-run --allow-read --allow-write --allow-net --allow-env --allow-sys=homedir,networkInterfaces` — this project's own README-documented flag set for the primary JSR install path — against a script calling `accessSync(path, X_OK)`: it threw the identical `NotCapable`. **This was never specific to `deno compile`.** The documented `deno run -A jsr:@tuesdaycrowd/mcp-comfyui` path is `-A`, so it was never actually exposed to end users that way, but anyone following the README's own long-form flag list — the one the README itself offers as the alternative to `-A` — would have hit the same `not_found` this entry opens with. Fixed together: `deno.json`'s `test` and `test:one` tasks now also grant `uid,gid` (they previously granted neither, which is *why* no test caught this — every real `defaultBinaryDeps()` call in the suite would have silently returned `false` from `isExecutable` even for a real, executable file); README's long-form flag list and `toolResult.ts`'s `permission_denied` message both now say `--allow-sys=…,uid,gid`; `binary.ts`'s `isExecutable` no longer swallows `NotCapable` as a plain `false` — it rethrows, so a missing grant now surfaces as `permission_denied` (the classification this project already had for exactly this error) instead of a misleading `not_found`; and a new `tests/binary.test.ts` case exercises the real `defaultBinaryDeps()` against a `chmod`-flipped temp file. That last one is the test that would have caught this — confirmed by running it against the *old* flag set (no `uid`/`gid`) and watching it fail on exactly the assertion this bug breaks, then confirming it passes with the fix. Also correcting this entry's own overstatement above: "every test that reaches `resolveComfyBinary` injects a fake `isExecutable`" should read *no test calls `defaultBinaryDeps()` against a real file* — the tests that reach it via `exec.ts`/`tools.ts` all set `COMFY_BIN`, which never calls `isExecutable` at all, rather than injecting a fake one. With every one of these changes in place: `deno task test` — **171 passed (753 steps), 0 failed**; `deno task typecheck` — zero errors; and the full `scripts/smoke-autolaunch.mjs` harness against the recompiled binary passed end to end again, `comfy_status.cli` still `{"source": "discovered", "path_repaired": true}`, the job completing in **70.9 s** this time and `local_paths` pointing at another real 1,223,071-byte 1024×1024 PNG (`z-image_00004_.png` — same workflow, same seed, same bytes as before). Criterion 2 was re-checked against the same recompiled binary and is unchanged, because `COMFY_BIN` never calls `isExecutable`: identical `comfy_unavailable` payload, still no `searched` key. ComfyUI was stopped again afterward.

**Second follow-up, same day: the previous follow-up's own overstatement was still wrong, and it hid a second, more serious regression.** Three tests in `tests/exec.test.ts` — `"falls back to comfy on PATH when COMFY_BIN is unset"`, `"a comfy already on PATH leaves the child's PATH untouched"`, and `"a comfy that is nowhere names every candidate that was tried"` — unset `COMFY_BIN` and do reach the real `defaultBinaryDeps()`; the paragraph above claiming otherwise was itself wrong, not merely imprecise. Two of them exercise it against a **real symlink** standing in for `comfy` on `PATH`, and both passed before this fix **for the wrong reason**: `isExecutable` returned `false` for that real, executable symlink too, resolution fell to `not_found`, and `spawn("comfy", …)` found the symlink via the OS's own `PATH` lookup regardless — the observable result matched a correct resolution by accident. (The third test's candidates are all nonexistent paths, so it never reached `accessSync` either way and was never touched by this bug.) One of the two also carries a `process.env.HOME = workdir` guard, commented "not belt-and-braces… without this the test does not fail cleanly" — a claim that was **false** under the bug (a real `~/.local/bin/comfy` on the machine running the suite would have reported `false` regardless of `HOME`, so the override changed nothing) and is genuinely true now.

**Worth stating plainly, since nothing above did:** one real configuration goes from working-by-accident to a hard failure under this fix. Deno, the stale documented flags, and a `comfy` genuinely reachable on `PATH` — before, `isExecutable` returned `false`, resolution still fell to `not_found`, and `spawn("comfy", …)` found it anyway via the OS's own lookup, so everything worked and only `cli.source` misreported. Now the first real `PATH` candidate throws immediately and every tool call that shells out fails `permission_denied` instead of quietly working. That is the fail-fast behaviour this rethrow exists to produce, not a bug in it, but it is a genuine behaviour change for that one configuration and deserves saying so rather than describing the fix only as "misleading `not_found` → accurate `permission_denied`".

**The more serious regression: `isVerdict` (`src/comfy/instance.ts`) did not know a `NotCapable` was terminal, and this is on the auto-launch path — the default.** `startLaunch` captures a `NotCapable` thrown during its `runComfy` call into the same polled `failure` state as any other cause, but `isVerdict` matched only `ComfyCliError` and `ComfyUnavailableError`. The poll loop therefore never recognised it as terminal and burned the full five-minute `DEFAULT_READY_TIMEOUT_MS` before reporting a generic `LaunchTimeoutError`, with the real cause surviving only as trailing `describeFailure` text — a strict regression from the *fast* `ComfyUnavailableError` an operator got before the rethrow existed. Fixed by adding a `failure.name === "NotCapable"` arm to `isVerdict` directly, in preference to restructuring `startLaunch`'s deliberate fire-and-poll design (documented at its own definition: not awaiting the launch is the whole point, since `comfy launch --background`'s own readiness signal is an unreliable log scrape) to await the failure instead — `isVerdict`'s entire job is already "is this terminal", a `NotCapable` unambiguously is one, and no other change was needed. `whichWorkspace`'s own catch of the same error, earlier in the same launch, stays silent by design (a missing `--output-directory` default must never fail a launch on its own) — the fault still reaches the operator because the identical, deterministic permission error recurs on `startLaunch`'s own `runComfy` call moments later, and now that call's failure is recognised. Pinned by a direct unit test of the now-exported `isVerdict` (`tests/instance.test.ts`) rather than a live `launchInstance` run: confirmed by reverting the fix and watching the new test fail exactly as predicted, then confirming it passes restored. A live integration reproduction through `launchInstance` is not possible in this suite: the exact condition that raises a genuine `NotCapable` needs a permission grant `deno task test` itself must hold for every other test here to exercise `defaultBinaryDeps()` correctly, so the two cannot coexist in one `deno test` process.

A new `tests/toolResult.test.ts` also now pins the classification the whole chain depends on: a `NotCapable` error yields `permission_denied`, and its advice message contains every token `deno.json`'s `compile` task actually grants under `--allow-sys` — read from the manifest, the same shape `server.test.ts` already uses to pin `SERVER_VERSION`. Before this, no test file imported anything from `toolResult.ts` at all, so this classification and its advice string had zero coverage between them; confirmed to fail when the advice string was mutated to drop `gid`.

Re-verified after all of the above: `deno task test` — **173 passed (754 steps), 0 failed**; `deno task typecheck` — zero errors; and `scripts/smoke-autolaunch.mjs` against the rebuilt-and-recompiled binary passed end to end again — `comfy_status.cli` still `{"source": "discovered", "path_repaired": true}`, the job completing in **76.2 s** and `local_paths` pointing at another real 1,223,071-byte 1024×1024 PNG (`z-image_00005_.png` — same workflow, same seed, same bytes as every prior run). Criterion 2 re-checked once more against the same binary: unchanged. ComfyUI was stopped again afterward.

**Third follow-up, same day: two accuracy corrections to the paragraphs directly above, caught by a third review pass.**

The `process.env.HOME = workdir` guard described two paragraphs up is misattributed there — it belongs to the **third** `tests/exec.test.ts` test (`"a comfy that is nowhere names every candidate that was tried"`, at line 389), not to either of the two symlink tests. The inertness claim attached to it is also half-wrong for the test it is actually on: that guard was **always** load-bearing, for a reason unrelated to this bug — its `searched` assertion checks for `join(workdir, ".local", "bin", "comfy")`, which needs `HOME` set to `workdir` regardless of what `isExecutable` does. What was inert under the bug is the specific hazard the guard's own comment names — shelling out to the developer's real, installed comfy-cli, since `isExecutable` answered `false` for that real binary too, whatever `HOME` said. That hazard is real now that `isExecutable` behaves correctly.

The "one configuration this rethrow turns from working-by-accident into a hard failure" paragraph understates its own blast radius by saying it hits "every tool call that shells out". `comfy_status` does **not** shell out, and is affected anyway: it calls `resolveComfyBinary(defaultBinaryDeps())` directly and unguarded (`src/tools.ts:1514`) to fill in its own `cli` block. In exactly the disclosed configuration, `comfy_status` returns `permission_denied` in place of its entire status report — no `target`, no `running`, no probe result — for the one tool whose own description says to call it first whenever another tool reports the server unreachable.

**Fourth follow-up, 2026-09-20: a final whole-branch review closed two branches that were dead, and the numbers above are superseded.**

**Two mutants survived the suite, and a reviewer found them by applying them rather than by reading.** Removing `binary.ts`'s `isFile()` guard — the one §3.1 of the design calls *"required, not decorative"* — left `tests/binary.test.ts` and `tests/exec.test.ts` fully green, so an executable **directory** named `comfy` would have resolved as `discovered` and failed at `spawn` with EACCES. And hoisting `withDefaultOutputDirectory`'s explicit-workspace early return, so that `MCP_COMFYUI_WORKSPACE` never yields `--output-directory` at all, left `instance`, `tools`, `server` and `outputs` tests **all** green — the feature's main configured path was unpinned. Both now have mutation-verified tests; both mutants die. Note the first reviewer named the wrong line for the second one (`whichWorkspace`'s early return, which the invocation-count test already covered); the implementer found the real one by testing directly rather than trusting the report.

**Three documentation surfaces this branch itself invalidated, which no single task owned and so no task review covered.** `README.md` still said `MCP_COMFYUI_WORKSPACE` "affects launching only … does not change how artifacts are reported" — the exact opposite of what the new `--output-directory` default does, in the section the README pushes an operator toward. `LaunchResult.warnings`' doc comment and `launch_comfyui`'s tool description both still enumerated contention as the only cause of a warning. And this file's own `fake-comfy-dispatch` table still listed seven variables when there are now eight — **the same defect the 2026-08-27 audit recorded and fixed once already**, which is why the row now carries the hazard sentence rather than just the name.

`CHANGELOG.md`'s `[Unreleased]` was also split into `Added`/`Changed`/`Fixed`: the `--allow-sys=uid,gid` upgrade action is something an existing operator must *do*, and buried under `Fixed` it read as "nothing to do".

**Final state of the branch:** `deno task test` — **175 passed (755 steps), 0 failed**; `deno task typecheck` — zero errors. The 171 and 173 figures above are accurate for the moments they record and are left as written.

**Two gaps parked deliberately, not solved.** The `--output-directory` warning is invisible on the **auto-launch path, which is the default**: `EnsureResult`'s `launched` arm declares no `warnings` field, so no caller can read one whatever it destructures. Contention warnings have always shared that fate, so it is a gap rather than a regression. And discovery's two hardcoded roots (`/opt/homebrew/bin`, `/usr/local/bin`) cannot be neutralised by faking `$HOME`, so on a machine with a Homebrew-installed comfy-cli one `tests/exec.test.ts` case could reach a real binary — closing it needs a production-side escape hatch that would be worse than the gap.
