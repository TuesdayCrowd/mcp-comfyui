...
**Status**: Complete — 39 tests, 11 mutants killed. The guard was verified against a stand-in server serving the captured Desktop `/system_stats` body (`outcome: "already_running"`, `desktopManaged: true`, and the CLI provably never spawned). Not yet exercised against the real Desktop, which was stopped during Stage 3.

**Known residual, deliberate:** a detection probe that *times out* reads as `running: false`, so a ComfyUI whose event loop is wedged by a heavy sampling step could pass the guard. The refusal path and comfy-cli's own `port_in_use` still catch the same-port case; only a launch onto a *different* port during such a stall slips through. Treating a timeout as "unknown, refuse" would block every launch behind a flaky probe, which is the worse default.

**The guard checks two addresses**, not one: the detection target and, when different, the host/port the startup arguments name. Landmine #8 is about VRAM and the shared model directory, not merely the port — a second instance on 8189 is exactly the forbidden thing.
