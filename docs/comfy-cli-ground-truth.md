...
15. **`convert_ui_to_api` silently drops a subgraph's input widget values.** For a workflow whose controls live on a subgraph node (`definitions.subgraphs[].inputs[]` wired to inner nodes), the conversion ignores the subgraph's own widget values and emits the **inner** nodes' stale ones. Measured on `audio_stable_audio_3_medium.json`: subgraph inputs `user_input`/`duration`/`seed` are wired to inner `PrimitiveStringMultiline.value`, `PrimitiveFloat.value` and `KSampler.seed`, yet the submitted graph carried the inner primitives' values — `PrimitiveFloat = 150` and a stock tropical-house prompt — so a request for "black metal, 60 seconds" produced **150 seconds of tropical house**. Linked inner inputs come out missing entirely (`EmptyLatentAudio` submitted with only `batch_size`).

    Two consequences. `comfy workflow slots` reports the inner addresses (`52/6.text`, `52/11.seconds`) as settable — **they are inert**, because the conversion overwrites or drops them — while the subgraph's real controls are not listed at all. And `set-slot` reports such an address as `applied`, so nothing anywhere signals that the value was discarded. **A subgraph workflow cannot currently be parameterised, or even faithfully executed, through comfy-cli.** ComfyUI's own frontend does its own conversion and is unaffected.
